...
class SomDetect:
    ...
    def detect_anomaly(
            self, label = None,
            level = .9,
            clt_test = "gai", mfdr = None, power = None,
            clt_map = False
    ):
        """
...
def main():
    ...
    parser.add_argument(
        ...
        type = float,
        default = "cltlind",
        help = "Significant level for the test"
    )
    # clt and cltlind
    parser.add_argument(
        "-o", "--overfit",
    ...
    som_anomaly.detect_anomaly(label=label,
    ...
    print("------------------------------------------")
    print("Normal quantile of %.3f" %ztest_opt)
